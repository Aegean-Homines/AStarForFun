...
void AStarSolver::ClearMap ( ) {
	// Basically reinitialize the node map minus allocation
...
{
	// boundary test
	if (target.first > height || target.second > width || start.first > height || start.second > width)
		return false;

	// Wall test
	if (mapJson[target.first][target.second] != 0)
		return false;
	 ...
	 * smooth the path
	 * 
	 * If bad performance = use prioritized list
	 */
